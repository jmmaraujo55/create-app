...
const [appname] = program.processedArgs;
...
function buildBaseFiles(baseDir, appname) {
  const envFile = fs.readFileSync(`${baseDir}/environment.ts`).toString();
  fs.mkdirSync(`${baseDir}/tmp`);
  fs.writeFileSync(
    `${baseDir}/tmp/environment.ts`,
    envFile.replace('"APP_NAME"', `"${appname}"`)
  );
  ...
  fs.copyFileSync(`${baseDir}/.gitignore.txt`, `${appname}/.gitignore`);

  fs.rmSync(`${baseDir}/tmp`, { recursive: true });
}
